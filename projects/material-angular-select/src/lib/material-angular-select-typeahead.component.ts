...
@Component({
  selector: 'material-angular-select-typeahead', // tslint:disable-line
  styleUrls: ['./material-angular-select.component.scss'],
  templateUrl: './material-angular-select-typeahead.component.html',
  encapsulation: ViewEncapsulation.None, // tslint:disable-line
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => MaterialAngularSelectTypeaheadComponent),
      multi: true,
    },
  ],
})

export class MaterialAngularSelectTypeaheadComponent implements OnInit, OnChanges, AfterViewInit, ControlValueAccessor {
  ...
  public menuKeyDown(event: KeyboardEvent) {
    ...
    switch (event.code) {
      ...
      case 'Enter':
        if (isVisible) {
          this.setCurrentValue(this.selectedDataArray[this.arrowkeyLocation]);
          this.closeMenu();
        } else {
          this.openMenu();
        }
